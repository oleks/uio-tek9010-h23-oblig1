...
const nCompletedElement = document.getElementById("n_completed") as HTMLSpanElement;
const timeElapsedElement = document.getElementById("time_elapsed") as HTMLSpanElement;
const scoreElement = document.getElementById("score") as HTMLSpanElement;

const fps_selector = document.getElementById("fps") as HTMLSelectElement;
let fps = parseInt(fps_selector.value);
fps_selector.addEventListener("change", function() { fps = parseInt(fps_selector.value); });

const stopAfter_selector = document.getElementById("stop_after") as HTMLSelectElement;
...
let tasks: { x: number; y: number, nearby_agents: number[] }[] = [];
let agents: { x: number; y: number, nearby_tasks: number[] }[] = [];

const nAgents_selector = document.getElementById("n_agents") as HTMLSelectElement;
let nAgents = parseInt(nAgents_selector.value);

const taskCapacity_selector = document.getElementById("task_capacity") as HTMLSelectElement;
...
const nTasks_selector = document.getElementById("n_tasks") as HTMLSelectElement;
...
function drawTickLabel(label : string, x : number, y : number) {
  ctx.font = '16px Arial';
  ctx.fillStyle = 'black';
...
function drawLabel(x: number, y: number, label: string) {
  ctx.font = '16px Arial';
  ctx.fillStyle = 'black';
  ctx.fillText(label, x + 15, y - 15);
}

function drawAgent(x : number, y : number, id: number) {
  ctx.beginPath();
...
function drawTask(x : number, y: number, id: number) {
  const color = 'black';
...
function find_ndxs<T>(arr: T[], condition: (element: T) => boolean): number[] {
